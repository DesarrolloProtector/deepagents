...
public sealed class MainWindow : Window
{
    ...
    private Control BuildLayout()
    {
        ...
        AddPanel(workspace, "Codex Output", _codexOutputEditor, row: 1, column: 0);
        AddPanel(workspace, "Review / Reviewer Prompt", _reviewPanel, row: 1, column: 1);
    ...
    private static void AddPanel(Grid parent, string title, TextBox editor, int row, int column)
    {
        Grid panel = new()
        {
    ...
    private void SetStatus(string message)
    {
        ...
        _status.Text = $"Repo: {repo} | Task mode: {_taskMode} | Selected context: {_selectedContextCount} | {_clipboardStatus} | {message}";
    }
