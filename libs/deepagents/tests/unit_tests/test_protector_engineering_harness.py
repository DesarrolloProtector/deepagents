...
def _pass_executor_reliability(monkeypatch) -> None:
    def pass_reliability(
        ...
        return cli._ExecutorReliabilityReport(
            ...
            checks=(
                cli._ExecutorReliabilityCheck(name="codex_cli_available", passed=True, detail="fake-codex"),
                ...
                cli._ExecutorReliabilityCheck(name="ph_local_workspace_check", passed=True, detail="ok"),
                ...
                cli._ExecutorReliabilityCheck(name="codex_sandbox_helper_available", passed=True, detail="ok"),
                cli._ExecutorReliabilityCheck(name="codex_model_execution_available", passed=True, detail="not checked"),
            ),
        )
...
def test_automation_readiness_allows_generic_only_narrow_visual_task(tmp_path: Path) -> None:
    ...
    assert (
        "No pack specialization was selected; generic runtime skills are acceptable because no pack-specific evidence is required." in rendered.text
    )
...
def test_cli_candidate_execute_interrupt_terminates_child_and_preserves_output(
    ...
    assert terminated[0][1] == "keyboard_interrupt"
    ...
    assert "Process tree status: process_tree_terminated" in stdout
    assert "- root_process_id: 12345" in stdout
    assert "- tracked_process_count: 2" in stdout
    assert "- termination_method: keyboard_interrupt" in stdout
    assert "- terminated_processes: 12345, 23456" in stdout
...
def test_cli_candidate_execute_interrupt_reports_termination_failure(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    stdout = capsys.readouterr().out
    assert "interrupted_by_operator" in stdout
    assert "termination_incomplete" in stdout
    assert "- resisted_processes: 12345" in stdout
    assert "Next recommended command:" not in stdout


def test_cli_candidate_execute_timeout_cleans_up_tree_and_reports_diagnostics(tmp_path: Path, monkeypatch, capsys) -> None:
    repo = _build_repo(tmp_path / "repo")
    candidate = tmp_path / "candidate.json"
    output = tmp_path / "codex-output.txt"
    payload = engineering.render_controlled_execution_plan(
    ...
    def fake_terminate(process: object, *, method: str) -> cli._ProcessTerminationReport:
        ...
        return cli._ProcessTerminationReport(
            root_pid=12345,
            tracked_pids=(12345, 23456, 34567),
            method=method,
            terminated_pids=(12345, 23456, 34567),
            resisted_pids=(),
        )
    ...
    stdout = capsys.readouterr().out
    assert "partial before timeout" in stdout
    assert output.read_text(encoding="utf-8") == "partial before timeout\n"
    assert "Candidate execution status: timeout" in stdout
    assert "- root_process_id: 12345" in stdout
    assert "- tracked_process_count: 3" in stdout
    assert "- termination_method: timeout" in stdout
...
def test_codex_cli_executor_preflight_success_uses_deterministic_workspace_checks(tmp_path: Path, monkeypatch) -> None:
    repo = _build_repo(tmp_path / "repo")
    ...
    def fail_run_codex_once(*args: object, **kwargs: object) -> cli._CodexExecutionResult:
        _ = args, kwargs
        msg = "deterministic executor preflight must not launch a model session"
        raise AssertionError(msg)
    ...
    assert [check.name for check in report.checks] == [
        ...
        "codex_model_execution_available",
    ]
...
def test_codex_cli_executor_preflight_reports_missing_executable(tmp_path: Path, monkeypatch) -> None:
    ...
    assert report.failing_checks[0].name == "codex_cli_available"
...
def test_codex_cli_executor_preflight_reports_local_workspace_read_failure(tmp_path: Path, monkeypatch) -> None:
    repo = _build_repo(tmp_path / "repo")
    codex_home = tmp_path / ".codex"
    codex_home.mkdir()
    ...
    def fail_run_codex_once(*args: object, **kwargs: object) -> cli._CodexExecutionResult:
        _ = args, kwargs
        msg = "workspace read failure must be detected before Codex model execution"
        raise AssertionError(msg)
...
def test_codex_cli_executor_preflight_reports_local_workspace_write_delete_failure(tmp_path: Path, monkeypatch) -> None:
    repo = _build_repo(tmp_path / "repo")
    codex_home = tmp_path / ".codex"
    codex_home.mkdir()
    monkeypatch.setattr(cli.sys, "platform", "win32")
    ...
    def fail_preflight_write(self: Path, data: str, *args: object, **kwargs: object) -> int:
        if self.name == "executor-preflight.tmp":
            msg = "write denied"
            raise OSError(msg)
        return original_write_text(self, data, *args, **kwargs)

    monkeypatch.setattr(Path, "write_text", fail_preflight_write)
...
def test_candidate_execute_no_longer_blocks_on_model_preflight_markers(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "Next review command:" in stdout
    assert len(calls) == 1
    assert "Local executor reliability preflight" not in calls[0]
    assert "Connector-only or remote repository fallback is explicitly disallowed" in calls[0]
    assert "Fix legacy onboarding path convergence" in calls[0]
...
def test_executor_status_and_candidate_execute_use_identical_environment_resolution(tmp_path: Path, monkeypatch) -> None:
    ...
    assert len(calls) == 1
    execute_env = calls[0]["kwargs"]["env"]
    assert execute_env["PATH"].split(";")[0] == report.runtime_path_prefix == str(helper.parent)
...
def test_cli_candidate_execute_blocks_before_candidate_prompt_when_preflight_fails(
    ...
    stdout = capsys.readouterr().out
    assert "Candidate execution refused: local executor reliability preflight failed." in stdout
    assert "codex_sandbox_helper_available: codex-windows-sandbox-setup.exe program not found" in stdout
    ...
    report = cli._ExecutorReliabilityReport(
        ...
        checks=(
            cli._ExecutorReliabilityCheck(
                ...
                passed=False,
                detail="ph direct local filesystem check failed.",
                suggested_fix="Fix local workspace access before running candidate automation.",
            ),
        ),
    )
...
def test_run_codex_once_timeout_terminates_child_process(tmp_path: Path) -> None:
    ...
    result = cli._run_codex_once((sys.executable, "-c", root_code), "prompt", timeout=1)

    child_pid = int(pid_file.read_text(encoding="utf-8"))
    assert result.returncode == 124
    assert result.timed_out
    assert result.termination_report is not None
    assert result.termination_report.method == "timeout"
    assert child_pid in result.termination_report.tracked_pids
    assert child_pid in result.termination_report.terminated_pids
    assert child_pid not in result.termination_report.resisted_pids
    assert not cli._is_pid_running(child_pid)


def test_run_codex_once_timeout_terminates_nested_descendant(tmp_path: Path) -> None:
    pid_file = tmp_path / "nested-pids.txt"
    grandchild_code = "import time\ntime.sleep(30)\n"
    child_code = (
        "import os, pathlib, subprocess, sys, time\n"
        f"path = pathlib.Path({str(pid_file)!r})\n"
    ...
    assert child_pid in result.termination_report.tracked_pids
    assert grandchild_pid in result.termination_report.tracked_pids
    assert not result.termination_report.resisted_pids
    assert not cli._is_pid_running(child_pid)
    assert not cli._is_pid_running(grandchild_pid)
